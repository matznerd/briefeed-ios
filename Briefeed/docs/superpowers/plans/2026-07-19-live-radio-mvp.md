...
`ContentView` defaults to `.radio` and mounts only the selected Radio, Brief, or Feed root through a `switch`. Do not retain hidden roots: a physical-device CPU termination proved that 0.1-second playback publications can otherwise recompute all three SwiftUI trees. Durable playback and queue state remains in shared services; root-local presentation state may reset across tab switches. Do not use a native `TabView` or rely on `.toolbar(.hidden, for: .tabBar)`: iOS 26 can re-expose that system bar and create a second active menu. Place the custom chrome after the flexible root content in the vertical layout. Do not overlay the chrome or use a fixed `49`-point padding: the content frame must end before `RadioTabRail`, followed by `MiniAudioPlayerV4`, so list rows cannot render under either control and the mini-player remains the bottom-most app control above the home indicator.

Keep high-frequency observation below the whole-screen root. Isolate lower-player observation in its own host, expose playback time to SwiftUI at most once per displayed second, publish/persist Radio entry progress at five-second boundaries, and retain exact force-saves for semantic and lifecycle transitions. Use value-based source navigation so archive candidates are derived only when the archive opens.
