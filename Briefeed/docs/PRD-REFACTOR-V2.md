...
> **Physical-device hardening amendment (July 20, 2026):** The first iPhone run exposed presentation and cold-launch gaps not represented by the fixture suite. Radio Home became a visible listening-state playlist, while source administration moved to Settings except for no-source recovery. The mini-player is a compact bottom-docked surface that continues through the home-indicator area, with transport on the right and speed, sleep, and scrubbing in one lower row. Production Radio restoration starts only after the first active scene so a transient launch `.inactive` phase cannot consume autoplay. Focused lifecycle and presentation suites, the expanded 16-test Radio UI suite, and a directly inspected final headless screenshot pass; the exact hardening build is installed and launched on the approved iPhone. Audible autoplay, resume, Lock Screen/Control Center, and sleep-under-lock remain owner-observed physical gates. Podcast ad detection remains separate future research in [`docs/research/2026-07-20-podcast-ad-skip-spike.md`](research/2026-07-20-podcast-ad-skip-spike.md).

> **Source-centric brief clarification (July 20, 2026):** Radio Home represents sources, not every fetched episode. Automatic restore and refresh keep at most one episode per source and choose that source's newest eligible update; a newer hourly bulletin replaces an older unplayed or restored automatic slot rather than following it in the same scan. An actively playing or user-paused episode is not interrupted by refresh, but the newer episode is available in the source archive. Hourly bulletin titles use a compact known network identity (`NPR`, `ABC`, `CBS`, or `CBC`) plus publication time and date formatted in the user's current time zone. Manual Next saves progress without marking completion; an automatic hourly edition is persisted as retired so reopening does not replay that same edition, and the retirement is replaced when its source publishes a newer edition. Daily and manually queued archive episodes defer instead, preserving their resume position. Selecting a source opens its descending episode archive, where prior retained episodes can be played immediately or explicitly queued for later. Explicit manual archive entries are the only permitted same-source queue exception. Partial refresh failures do not interrupt or add a banner to Radio Home; only affected rows in Radio Sources receive an issue indicator. The Radio list and source archives must terminate above, never underneath, the persistent navigation and player chrome. The compact `RadioTabRail` is the sole Radio/Brief/Feed navigation surface; the root must not also instantiate a native system tab bar.

> **Source-centric verification receipt:** The single-rail correction passes a signed device build, strict code-sign verification, and a signed build-for-testing of the app, unit-test, and UI-test targets. That exact app is installed and launched on the approved iPhone without clearing its data. Simulator runtime verification for this exact follow-up remains deferred because the shared host is still under severe scheduler pressure; do not infer a runtime UI-test pass from compilation. Owner phone inspection of the sole custom rail and a later healthy-fleet rerun remain open.

> **Playback CPU termination correction (July 20, 2026):** Physical iPhone reports `Briefeed.cpu_resource_fatal-2026-07-20-220858.ips` and `Briefeed.cpu_resource_fatal-2026-07-20-221445.ips` show iOS terminating the app for 89-99 percent sustained CPU, not a TTS or FluidAudio crash. Symbolication tied the hot path to SwiftUI/AttributeGraph recomputation across simultaneously mounted Radio, Brief, and Feed roots during 0.1-second playback updates. The root now mounts only the selected section; player presentation time publishes at most once per displayed second; durable Radio progress publishes and persists at five-second buckets while exact state remains force-saved on pause, seek, Next, interruption, route loss, background, and termination; and source-archive candidates are constructed only on navigation. The signed fix is installed on the approved iPhone and remained running beyond the prior kill window with no new crash report. The focused player-presentation suite passes 19/19 and the new Radio progress-publication regression passes; one unrelated pre-existing playback-state test remains open, and simulator runtime verification remains deferred under critical host swap pressure. Duplicate low-level progress timers are follow-up GitHub issue #20.
...
> **Radio reopen and background freshness correction (July 25, 2026):** Radio Home now reports its own appearance to the lifecycle driver. Reopening that tab after at least 60 seconds requests one forced enabled-source refresh, while the debounce prevents SwiftUI remounts from duplicating work; refresh reconciliation still does not interrupt active audio or create another autoplay opportunity. The visible latest-source row may advance to a newly published NPR bulletin while the mini player remains attached to the older NPR bulletin already in progress. Briefeed also registers `Matznerd.Briefeed.radio-refresh` as an iOS `BGAppRefreshTask`, requests a best-effort refresh window beginning 45 minutes later, and re-arms the request whenever the task runs or the app enters background. Background work uses the stale policy rather than unconditional requests. iOS chooses whether and when that task runs, so cold launch, foreground return, and Radio Home appearance remain the deterministic freshness gates.
